...
static double kek_console_debug_last_print = 0.0;
...
static qboolean kek_IsPlayerAlive( cl_entity_t *ent )
{
	...
	if( ent->model->type == mod_bad )
		return false;

	// ДОПОЛНИТЕЛЬНАЯ ПРОВЕРКА: spectator режим
	if( ent->curstate.spectator )
		return false;

	// ПРОВЕРКА НА МЕРТВОГО ИГРОКА:
	// Используем комбинацию признаков, характерных для мертвых игроков:
	// - Нет оружия И тип движения NONE (как в коде рендеринга мертвых тел)
	// - Твердость NOT (нет взаимодействия с окружением)
	// - Spectator режим
	if( (ent->curstate.weaponmodel == 0 && ent->curstate.movetype == MOVETYPE_NONE) ||
...
static void kek_DrawAimbotFOV( void )
{
	...
	radius = (fov_value / 90.0f) * vp_width * 0.5f;
...
static void kek_Aimbot( ref_viewpass_t *rvp )
{
	...
	for( i = 1; i < tr.max_entities; i++ )
	{
		...
		// Check if player is alive (bypass anti-cheat that sets SOLID_NOT)
